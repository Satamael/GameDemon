...
void FGameOver (bool* GameOver, Player pers, Player enemy, Player ally, Player enemy_old, Player ally_old, Player pers_old, int screenH, int screenW, HDC GameOverPicBad, int finish,  bool BackMenu, bool StartGame, menupics menu1);
 void Game (Wall* dark, Player pers, Player enemy, Player ally,
bool GameOver, int screenH, int screenW);
...
int main()
{
    ...
    Wall dark[nWallD];
    menupics menu1 = {txLoadImage("menu/menuEN.bmp"), txLoadImage("menu/autor.bmp"), txLoadImage("menu/prehistoryEN.bmp"), txLoadImage("menu/ContractTypeEN.bmp")};
    int speed = pers.speed;
    char ForSpeed[20];
    //sprintf(ForSpeed, "%d", speed);
    ...
    if (StartGame==true) {
        Game (dark, pers,enemy, ally, GameOver, screenH, screenW);
    }
  ...
  void Game (Wall* dark, Player pers, Player enemy, Player ally,
    ...
    while (GameOver != true)
    {
        txSetColor(TX_BLACK);
            ...
            txEnd();
            FGameOver (&GameOver,pers,enemy,ally,enemy_old,
            ally_old,pers_old,screenH,screenW,GameOverPicBad, 500, &StartGame, menu1);
            txSleep(20);
    }
